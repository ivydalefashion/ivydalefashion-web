...
export class BlogService {
  ...
  async findAll(status?: BlogStatus, tagSlug?: string) {
    const where: any = {};
  ...
  async create(dto: CreatePostDto) {
    ...
    return this.prisma.blogPost.create({
      data: {
        ...data,
        publishedAt: data.status === BlogStatus.PUBLISHED ? new Date() : undefined,
      ...
      data: {
        ...
        publishedAt:
          data.status === BlogStatus.PUBLISHED
            ? (await this.prisma.blogPost.findUnique({ where: { id } }))?.publishedAt ?? new Date()
            : undefined,
        tags: tagIds
          ? {
              deleteMany: {},
